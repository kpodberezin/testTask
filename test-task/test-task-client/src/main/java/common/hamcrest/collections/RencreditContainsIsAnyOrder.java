...
public class RencreditContainsIsAnyOrder extends RencreditCollectionCondition {
    protected final List<String> expectedTexts;

    public RencreditContainsIsAnyOrder(String... expectedTexts) {
        this(asList(expectedTexts));
    }

    public RencreditContainsIsAnyOrder(List<String> expectedTexts) {
    ...
    public boolean apply(List<WebElement> elements) {
        for (String expectedText : expectedTexts) {
            if (elements.stream().filter(webElement -> webElement.getText().contains(expectedText)).collect(Collectors.toList()).size() != 1) {
                return false;
            }
        }
        return true;
